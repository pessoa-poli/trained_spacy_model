...
def build_doc_bin(data, is_test_data=False):    
...
def save_doc_bin(save_file_path):
    doc_bin = DocBin(docs=all_docs)
...
if __name__ == "__main__":   
    ...
    start = time.time()
    build_doc_bin(data=TRAIN_DATA)

    print('Creating a DocBin for the TEST_DATA')
    build_doc_bin(data=TEST_DATA, is_test_data=True)

    save_doc_bin(save_file_path='./doc_bins/all_data.doc_bin.spacy')
    end = time.time()
